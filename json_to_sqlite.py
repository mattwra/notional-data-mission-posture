...
def create_mpd_table(cursor):
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS mpd_data (
            ...
            ADDRESS_ADMIN_1 VARCHAR(128),
            ADDRESS_ADMIN_1_TYPE VARCHAR(128),
            DFP VARCHAR(256),
...
def insert_mpd_data(cursor, data):
    ...
    insert_query = '''
        INSERT INTO mpd_data (
            ...
            FUNCTIONAL_ROLE, COUNTRY, COUNTRY_CODE, NIPF_PRIORITY, DOMAIN, FTE,
            EMPLOYEE_SKILL_COMMUNITY, MISSION_ELEMENT, LOCATION_SPECIFIC,
            ADDRESS_ADMIN_1, ADDRESS_ADMIN_1_TYPE, DFP, WORK_ROLE, CITY,
            CIMPL_RANK_CATEGORY, ASSIGNED_ORG, STATUS, SITE, LOE_JUSTIFICATION,
            REGION, AFFILIATION_TYPE, ACTIVITY_DAF, CRITICAL_SKILLS,
            DOMAIN_TWO_PLUS_THREE, SITE_RESILIENCE, TOKENS
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
                 ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    '''
    ...
    for record in data:
        try:
            cursor.execute(insert_query, (
                ...
                record['ADDRESS_ADMIN_1'],
                record['ADDRESS_ADMIN_1_TYPE'],
                record['DFP'],
                record['WORK_ROLE'],
                record['CITY'],
